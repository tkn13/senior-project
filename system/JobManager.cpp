...
JobResult JobManager::processJobs(JobMap &prev_map, JobMap &curr_map)
{
    ...
    {
        if (neg_status.find(it->second.status) != neg_status.end())
        {
    ...
    for (const auto &[job_id, curr_job] : curr_map)
    {
        ...
        if (prev_map.find(job_id) == prev_map.end())
        {
            ...
            returnValue.to_update.push_back(job_id);
        }
        else if (prev_map[job_id].status != curr_job.status)
        {
        ...
        returnValue.new_prev_map[job_id] = curr_job;
    }

    // 3. Handle "Leftover" jobs (Jobs in prev_map that are no longer in curr_map)
    for (const auto &[job_id, prev_job] : prev_map)
    {
